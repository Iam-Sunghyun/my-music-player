...
const EQ_FREQUENCIES = [60, 170, 310, 600, 1000, 3000, 6000]; // 주파수 음역대
...
export default function useAudioContext(audioElement) {
  ...
  // 음악을 선택한 상태에서 audioContext와 analyser가 없는 경우 AudioContext 및 AnalyserNode를 초기화하는 effect -> 사용자 상호작용 없이 AudioContext 생성 혹은 활성화 불가능하므로 현재 음악 선택 후 AudioContext 생성하도록 함.
  useEffect(() => {
  ...
  useEffect(() => {
    if (audioContext && analyser && panner && filters) {
      ...
      if (!audioSourceRef.current) {
        ...
        source.connect(filters[0]); // 첫 번째 필터에 오디오 소스 연결
